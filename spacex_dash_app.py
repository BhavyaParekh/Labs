...
def update_pie_chart(site):
    if site == 'ALL':
        ...
        fig = px.pie(filtered_df, names='class', title=f'Success Launches at {site}')
    return fig
...
def update_scatter_chart(site, payload_range):
    ...
    if site == 'ALL':
        ...
        filtered_site_df = filtered_df[filtered_df['Launch Site'] == site]
        fig = px.scatter(filtered_site_df, x='Payload Mass (kg)', y='class',
                         title=f'Payload vs Success Rate for {site}',
                         labels={'Payload Mass (kg)': 'Payload Mass (kg)', 'class': 'Success'})
    return fig
